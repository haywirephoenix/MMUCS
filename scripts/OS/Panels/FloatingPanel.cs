...
/// <summary>
/// Base class for all floating panels in MMUCS.
/// Handles dragging, clamping to canvas, z-ordering, and collapse.
/// Inherit from this for every panel type.
/// </summary>
///
...
public partial class FloatingPanel : Control
{
    ...
    [Export] public Control _contentRoot; // subclasses populate this
    ...
    [Export] public Control _canvas; // parent canvas, set on ready
    ...
    private void SubscribeHandles()
    {
        ...
        {
            // Direct lookup via pre-compiled NodePath
            if (GetNode<Control>(handleData.Path) is { } handle)
            {
                // Capture the direction for the lambda
                Vector2 capturedDir = handleData.Direction;
                handle.GuiInput += (ev) => _OnHandleInput(ev, capturedDir);
            }
            else
            {
                GD.PushWarning($"{Name}: missing resize handle at '{handleData.Path}'");
            }
        }
    }
    ...
    private void _OnHandleInput(InputEvent @event, Vector2 direction)
    {
        ...
        // 1. Handle Initial Click / Release
        if (@event is InputEventMouseButton mb && mb.ButtonIndex == MouseButton.Left)
        {
            ...
            else
            {
                ...
                Size = finalSize;
                Position = Position.SnapVector(); // Final cleanup for position
            }
        }
        ...
        if (@event is InputEventMouseMotion && _resizing)
        {
            ...
            // We calculate the change based on the direction vector
            // If direction is -1 (Left/Top), we subtract movement from size and add to position
            // If direction is 1 (Right/Bottom), we simply add movement to size
            ...
            if (_currentDirection.X != 0)
            {
                ...
                // If dragging Left, we must move the X position
                if (_currentDirection.X < 0)
                {
                    // Calculate how much we actually resized (clamped by MinSize)
            ...
            if (_currentDirection.Y != 0)
            {
                ...
                // If dragging Top, we must move the Y position
                if (_currentDirection.Y < 0)
                {
    ...
    private void _OnTitleBarInput(InputEvent @event)
    {
        ...
        if (@event is InputEventMouseMotion && _dragging)
        {
            ...
            // Normalize ranges (prevents crash)
            if (minX > maxX)
            {
    ...
    {
        // Other panels sink — only the requesting panel rises (handled locally via MoveToFront)
    }
    ...
    private void _FitToContents()
    {
        ...
        // Respect your minimum panel size
